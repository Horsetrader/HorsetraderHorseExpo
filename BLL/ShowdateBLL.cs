...
namespace BLL
{
    public class ShowdateBLL
    {
        ...
        public ShowdateBLL(int orderNumber)
        {
        ...
        private void loadObject(DataSet dsShowdate)
        {
            if (dsShowdate.Tables.Count > 0)
            {
                if (dsShowdate.Tables[0].Rows.Count > 0)
                {
                    ...
                    Foto_File = string.IsNullOrEmpty(drShowdate["Foto_File"].ToString()) ? string.Empty : drShowdate["Foto_File"].ToString();
                }
    ...
    {
        public int Approval_Number { get; set; }
